...
const main = create(
  'main',
  '',
  [create('h1', 'title', 'RS School Virtual Keyboard 2022'),
    create('p', 'hint', 'Created for Windows.'),
    create('p', 'hint', 'Для фиксирования кнопки shift/Ctrl необходимо удерживать левую кнопку мышки и сместить курсор за пределы кнопки'),
    create('p', 'hint', 'To fix the shift/Ctrl button, hold down the left mouse button and move the cursor outside the button'),

    create('p', 'hint', '____________________________________________________________________________'),
    create('p', 'hint', 'Используйте левый <kbd> Ctrl </kbd> + <kbd> Alt </kbd> для переключения языка. Последний язык сохраняется в localStorage'),
...
export default class Keyboard {
  ...
  preHandleEvent = (e) => {
    ...
    if (!code.match(/CapsLock|Shift|Control/)) {
  ...
  handleEvent = (e) => {
    ...
    // hide keyboard
    if (code.match(/Escape/) && !this.keyEsc) {
      this.keyEsc = true;
      this.closeKeyboard(true);
    } else if (code.match(/Escape/) && this.keyEsc) {
      this.keyEsc = false;
